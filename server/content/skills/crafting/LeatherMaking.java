...
static int INTERFACE = 4429;
static int FRAME = 1746;
		...
		for (final leatherData l : leatherData.values()) {
			...
			if (leather == l.getLeather()) {
				...
				if (l.getLeather() == 1743) {
					c.getPA().itemOnInterface(FRAME, 150, 1131);
					c.getPA().sendFrame164(INTERFACE);
					c.getPA().sendFrame126("Hardleather body", 2799);
					...
					CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
						...
						public void execute(CycleEventContainer container) {
							if (c.playerSkilling[12] == true) {
								...
								if(Misc.random(3) == 0) // Thread lost
								c.getItems().deleteItem(1734, c.getItems().getItemSlot(1734), 1);
								
								//c.getItems().deleteItem(1733, c.getItems().getItemSlot(1733), 1);
