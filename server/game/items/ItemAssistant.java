...
public class ItemAssistant {
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName2.startsWith("2h")) {
			c.setSidebarInterface(0, 4705);
			c.getPA().sendFrame246(4706, 200, Weapon);
			c.getPA().sendFrame126(WeaponName, 4708);
	...
	public void getRequirements(String itemName, int itemId) {
		c.runecraftingLevelReq = c.attackLevelReq = c.defenceLevelReq = c.strengthLevelReq = c.rangeLevelReq = c.magicLevelReq = 0;
		...
		if(itemName.contains("maple")) {
		    c.rangeLevelReq = 30;
		}
		if(itemName.contains("oak")) {
		    c.rangeLevelReq = 5;
		}
		if(itemName.contains("willow")) {
		    c.rangeLevelReq = 20;
		}
		...
		if(itemName.contains("black")) {
			if(!itemName.contains("hide") && !itemName.contains("knife") && !itemName.contains("dart") && !itemName.contains("javelin") && !itemName.contains("thrownaxe")) {
		...
		if(itemName.contains("dragon longsword")) {
			if (!itemName.contains("nti-") && !itemName.contains("fire")) {
				c.attackLevelReq = c.defenceLevelReq = 60;
				return;
			}
		...
		if(itemName.contains("dragon battle")) {
			if (!itemName.contains("nti-") && !itemName.contains("fire")) {
				c.attackLevelReq = c.defenceLevelReq = 60;
				return;
			}
		}
		if(itemName.contains("dragon chainbody")) {
			if (!itemName.contains("nti-") && !itemName.contains("fire")) {
				c.attackLevelReq = c.defenceLevelReq = 60;
				return;
			}
		}
		if(itemName.contains("dragon med")) {
			if (!itemName.contains("nti-") && !itemName.contains("fire")) {
				c.attackLevelReq = c.defenceLevelReq = 60;
				return;
			}
		}
		if(itemName.contains("dragon sq")) {
			if (!itemName.contains("nti-") && !itemName.contains("fire")) {
				c.attackLevelReq = c.defenceLevelReq = 60;
				return;
			}
	...
	public boolean wearItem(int wearID, int slot) {
		...
		synchronized(c) {
			if(c.tutorialprog == 22) {
				...
				(c, "Clothes, armour, weapons and many other items are equipped",
		...
		//	c.setSidebarInterface(0, -1);// worn
				...
				(c, "",
						...
						"Combat interface");
				Tutorialisland.chatbox(c, 6179);
			Tutorialisland.flashSideBarIcon(c, 0);
			Tutorialisland.TII(c,50, 11);
			...
			if(c.playerItems[slot] == (wearID+1)) {
				...
				if(wearID == 6857 || wearID == 6859 || wearID == 6861 || wearID == 6863 || wearID == 1702) {
					targetSlot = 2;
				}
				if(wearID == 2665 || wearID == 1037) {
					targetSlot = 0;
				}
				if (wearID == 2661 || wearID == 6394 || wearID == 6402 || wearID == 7392) {
					targetSlot = 4;
				}
				if (wearID == 2663 || wearID == 6396 || wearID == 6404) {
					targetSlot = 7;
				}
				/*
				 	0 hat
				 	...
				 	13 arrows
				 */
				if (wearID == 1127 && c.dragSlayer < 10 || wearID == 1135 && c.dragSlayer < 10) {
				    c.sendMessage("You must complete the Dragon Slayer quest in order to wear this.");
				    return false;
				}
			...
			if (Item.itemStackable[c.playerItems[fromSlot]-1] || c.playerItemsN[fromSlot] > 1) {
				...
				if (!alreadyInBank && freeBankSlots() > 0) {
					...
					if ((c.bankItemsN[toBankSlot] + amount) <= Config.MAXITEM_AMOUNT && (c.bankItemsN[toBankSlot] + amount) > -1) {
						...
						c.sendMessage("Bank is full. Please become a member to get more.");
						return false;
					}
