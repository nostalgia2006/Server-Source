package server.game.objects.doors;
...
public class Doors {
	...
	public boolean handleDoor(final Client c, int id, int x, int y, int z) {

		final Doors d = getDoor(id, x, y, z);
		...
		if(id == 1804 &&!c.getItems().playerHasItem(983)) {
			c.sendMessage("You need a key to open this door.");
			return false;
		}
		if(id == 1533 && x == 2932 && y == 3213) {
		c.membersonly();
		return false;
		}
	...
	public void load() {
		...
		System.out.println("[Stage 6] "+ doors.size() +" doors have been loaded in "+ (System.currentTimeMillis() - start) +"ms...");
	}
