...
public class PlayerAssistant{
	...
	**/
	public void loadQuests() {		
		...
		c.getPA().sendFrame126("Free Quests:", 663);
		if (c.blackK == 0) {
		c.getPA().sendFrame126("Black Knights' Fortress", 7332);
		} if (c.blackK >=1 && c.blackK <= 5) {
		c.getPA().sendFrame126("@yel@Black Knights' Fortress", 7332);
		} if (c.blackK == 6) {
		c.getPA().sendFrame126("@gre@Black Knights' Fortress", 7332);
		}		
		if (c.cooksA == 0) {
		...
		c.getPA().sendFrame126("@yel@Demon Slayer", 7334);
		} if (c.demonS == 6) {
		c.getPA().sendFrame126("@gre@Demon Slayer", 7334);
		}
		if (c.doricsQ == 0) {
...
/*/
	...
	public void walkableInterface(int id) {
		if (c.lastWalkableInterface == id) {
			return;
		}
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrame(208);
			c.getOutStream().writeWordBigEndian_dup(id);
			c.lastWalkableInterface = id;
	...
	public void magicOnItems(int slot, int itemId, int spellId) {
		...
		case 1162: // low alch
			if(System.currentTimeMillis() - c.alchDelay > 1000) {
				if (c.playerLevel[c.playerMagic] <= 20) {
					c.sendMessage("You need a magic level of at least 21 to cast this spell.");
					break;
				}
				if(!c.getItems().playerHasItem(554, 3) && !c.getItems().playerHasItem(561, 1)){
				...
				c.getItems().deleteItem(554, 3); c.getItems().deleteItem(561, 1);
		...
		case 1178: // high alch
			if(System.currentTimeMillis() - c.alchDelay > 2000) {
				if (c.playerLevel[c.playerMagic] <= 54) {
					c.sendMessage("You need a magic level of at least 55 to cast this spell.");
					break;
				}
				...
				c.getPA().sound(223);
				c.getItems().deleteItem(554, 5); c.getItems().deleteItem(561, 1);
				c.getItems().deleteItem(itemId, slot, 1);
	...
	public void levelUp(int skill) {
		...
		case 0:
			...
			sendFrame164(6247);
			Music.sendQuickSong(c,105, 33); // level up bitches!
		...
		case 1:
			...
			sendFrame164(6253);
			Music.sendQuickSong(c,88, 20); // level up bitches!
			break;
		...
		case 2:
			...
			sendFrame164(6206);
			Music.sendQuickSong(c,80, 20); // level up bitches!
		...
		case 3:
			...
			sendFrame164(6216);
			Music.sendQuickSong(c,94, 20); // level up bitches!
		...
		case 4:
			...
			sendFrame164(4443);
			Music.sendQuickSong(c,96, 20); // level up bitches!
		...
		case 5:
			...
			sendFrame164(6242);
			Music.sendQuickSong(c,83, 23); // level up bitches!
			break;
		...
		case 6:
			...
			sendFrame164(6211);
			Music.sendQuickSong(c,102, 24); // level up bitches!
			break;
		...
		case 7:
			...
			sendFrame164(6226);
			Music.sendQuickSong(c,89, 20); // level up bitches!
		...
		case 8:
			...
			sendFrame164(4272);
			Music.sendQuickSong(c,92, 23); // level up bitches!
			break;
		...
		case 10:
			...
			sendFrame164(6258);
			Music.sendQuickSong(c,85, 25); //fishing
			break;
		...
		case 11:
			...
			sendFrame164(4282);
			Music.sendQuickSong(c,67, 20); // level up bitches!
			break;
		...
		case 12:
			...
			sendFrame164(6263);
			Music.sendQuickSong(c,100, 20); // level up bitches!
			break;
		...
		case 13:
			...
			sendFrame164(6221);
			Music.sendQuickSong(c,78, 20); // level up bitches!			
			break;
		...
		case 14:
			...
			sendFrame164(4416);
			Music.sendQuickSong(c,95, 20); // level up bitches!
			break;
		...
		case 20:
			...
			sendFrame164(4267);
			Music.sendQuickSong(c,66, 25); // level up bitches!
			break;
		}
		if(totalLevel >= 1490)
			c.sendMessage("@red@Well done! You've achieved the highest total level possible on free worlds!");
		c.dialogueAction = 0;
		c.nextChat = 0;
		}
