...
public abstract class Player extends Mobile {
    ...
    public boolean cantClimbLadder, playerisSmelting, stopPlayerPacket = false,
	    ...
	    guildDoor, hardbodies = false, spinningPlate, soundEnabled = true;
    // random events
    ...
    public boolean DSOption, DSOption2, DSOption3, DSOption4, DSOption5, DSOption6, DSOption7,
    			...
    			incantationOption;
    public int leatherType = -1, blackK, demonS, Incantation, ernestC, goblinD, piratesT,
	    princeR, romeoJ, shieldA, cooksA, dragSlayer, impC, knightS,
	    restlessG, runeM, sheepS, lastGrab = 0, witchspot, vampireslay,
	    ...
	    strengthLevelReq, rangeLevelReq, magicLevelReq, followId,
	    skullTimer, nextChat = 0, talkingNpc = -1, dialogueAction = 0,
	    ...
	    doricsQ = 0, tutorialisland = 0, teleGrabItem, teleGrabX, teleGrabY, duelCount,
     ...
     */
    public int entTimer, mimeTimer = 0, failure,mimeEmote, mimeevent, fishNPC, whirlTime, expGained
	    , mimeSuccess, mimeFail;
    ...
    public final int[][] MAGIC_SPELLS = {
	    ...
	    { 1177, 53, 711, 138, 139, 140, 15, 31, 556, 3, 557, 4, 560, 1, 0,
		    0, 1007, 1005 }, // earth blast
	    { 1181, 59, 711, 129, 130, 131, 16, 35, 556, 4, 554, 5, 560, 1, 0,
		    0 }, // fire blast
	    // {1183,62,727,158,159,160,17,36,556,5,565,1,0,0,0,0}, // wind wave
	    // {1185,65,727,161,162,163,18,37,556,5,555,7,565,1,0,0}, // water
	    // wave
	    // {1188,70,727,164,165,166,19,40,556,5,557,7,565,1,0,0}, // earth
	    // wave
	    // {1189,75,727,155,156,157,20,42,556,5,554,7,565,1,0,0}, // fire
	    // wave
	    ...
	    // guthix
	    // {1192,60,811,0,0,78,20,60,554,4,565,2,556,1,0,0}, // flames of
	    // zammy
    ...
    public boolean forcedChatUpdateRequired, tradeAccepted, goodTrade, inTrade,
    ...
    /*
     * protected void appendPlayerAppearance(Stream str) { //synchronized(this)
     * { playerProps.currentOffset = 0;
     * 
     * playerProps.writeByte(playerAppearance[0]);
     * 
     * playerProps.writeByte(headIcon); playerProps.writeByte(headIconPk);
     ...
     * playerEquipment[playerHat]); } else { playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerCape] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerCape]); } else { playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerAmulet] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerAmulet]); } else { playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerWeapon] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerWeapon]); } else { playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerChest] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerChest]); } else {
     * playerProps.writeWord(0x100+playerAppearance[2]); }
     * 
     * if (playerEquipment[playerShield] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerShield]); } else { playerProps.writeByte(0); }
     * 
     * if (!Item.isFullBody(playerEquipment[playerChest])) {
     * playerProps.writeWord(0x100+playerAppearance[3]); } else {
     * playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerLegs] > 1) { playerProps.writeWord(0x200 +
     ...
     * playerProps.writeWord(0x100 + playerAppearance[1]); } else {
     * playerProps.writeByte(0); }
     * 
     * if (playerEquipment[playerHands] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerHands]); } else {
     * playerProps.writeWord(0x100+playerAppearance[4]); }
     * 
     * if (playerEquipment[playerFeet] > 1) { playerProps.writeWord(0x200 +
     * playerEquipment[playerFeet]); } else {
     * playerProps.writeWord(0x100+playerAppearance[6]); }
     * 
     * if (playerAppearance[0] != 1 &&
     * !Item.isFullMask(playerEquipment[playerHat])) {
     * playerProps.writeWord(0x100 + playerAppearance[7]); } else {
     * playerProps.writeByte(0); }
     * 
     ...
     * (getLevelForXP(playerXP[0])) + (double) (getLevelForXP(playerXP[2])));
     * 
    ...
    synchronized void postProcessing() {
