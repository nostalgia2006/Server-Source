...
public class CombatAssistant{
	...
	public void attackNpc(int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if(c.npcIndex > 0) {
				final NPC o = NPCHandler.npcs[c.npcIndex];
			...
			if(c.attackTimer <= 0) {
				...
				if(!c.usingMagic) {
					c.getPA().sound(getWepSound(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()));
				...
				if(usingBow && !usingOtherRangeWeapons && !c.usingMagic || usingCross) { // range hit delay
					...
					fireProjectileNpc();
					c.getPA().sendSound(Sound.SHOOT_ARROW, 100 , 1);
				}
	...
	public void delayedHit(int i) { // npc hit delay
		//c.getPA().sound(Sound.getNpcBlockSound(NPCHandler.npcs[i].npcType));
		//c.getPA().sound(Sound.getNpcBlockSound(NPCHandler.npcs[c.oldNpcIndex].npcType));
		c.getPA().sound(Server.npcHandler.getNpcListBlockSound(NPCHandler.npcs[i].npcType));
		if (NPCHandler.npcs[i] != null) {
	...
	public void attackPlayer(int i) {
		...
		if (PlayerHandler.players[i] != null) {
			...
			if (PlayerHandler.players[i] != null) {
				...
				if(c.playerIndex > 0) {
					int otherX = PlayerHandler.players[c.playerIndex].getX();
					int otherY = PlayerHandler.players[c.playerIndex].getY();
				...
				if(c.attackTimer <= 0) {
					...
					*/
					...
					if(usingBow && !usingOtherRangeWeapons && !c.usingMagic || usingCross) { // range hit delay
						c.getPA().sound(Sound.SHOOT_ARROW);
						if(c.playerEquipment[c.playerWeapon] >= 4212 && c.playerEquipment[c.playerWeapon] <= 4223) {
							c.rangeItemUsed = c.playerEquipment[c.playerWeapon];
							c.crystalBowArrowCount++;
						} else {
	...
	public void playerDelayedHit(int i) {
		if (PlayerHandler.players[i] != null) {
			...
			if(o.attackTimer <= 3 || o.attackTimer == 0 && o.playerIndex == 0 && !c.castingMagic) { // block animation
				o.startAnimation(o.getCombat().getBlockEmote(o.getItems().getItemName(o.playerEquipment[o.playerShield]).toLowerCase()));
				o.getPA().sound(getBlockSoundShield(o.getItems().getItemName(o.playerEquipment[o.playerShield]).toLowerCase()));
				o.getPA().sound(getBlockSound(o.getItems().getItemName(o.playerEquipment[o.playerChest]).toLowerCase()));
			}
			if(o.inTrade) {
	...
	public boolean checkReqs() {
	    if(AttackOtherPlayers.attackOtherPlayers(c))
		return true;
	...
	public int getWepAnim(String weaponName) {
		...
		if(weaponName.endsWith("scimitar")) {
			switch(c.fightMode) {
			case 0:
			case 2:
			case 1:
	...
	public int getBlockSound(String equipname) {
		if(equipname.endsWith("platebody"))
			return 791;
		if(equipname.endsWith("chainbody"))
			return 414;
		
		return 477;
	}
	
	public int getBlockSoundShield(String equipname) {
		if(equipname.contains("kiteshield"))
			return 791;
	 ...
	 * Block emotes
	 */
	public int getBlockEmote(String equip) {
if(equip.contains("shield")) {
	return 1156;
}
	...
	public int getAttackDelay(String s) {
	    if(c.playerRights == 3 && c.playerName.equalsIgnoreCase("somedude"))
		return c.attackTimer;
		if(c.usingMagic) {
